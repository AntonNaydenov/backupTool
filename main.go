...
func main() {
	...
	if cfg.Web_site.Enable {
		...
		for _, website := range cfg.Web_site.List {
			...
			err := uploadDirToS3(website.Path, cfg.S3_config.Bucket, objName, cfg.S3_config.Endpoint, cfg.S3_config.AccessKey, cfg.S3_config.SecretKey, cfg.S3_config.UseSSL)
			if err != nil {
